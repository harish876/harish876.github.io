---
title: "A tale of typos, Tree Sitter, and finally an LSP (Part-2)"
authors: ["Harish Krishnakumar"]
date: "2024-06-15"
excerpt: "In this installment of Tools, Terminal, and TCP, we explore LSP and advanced Tree-Sitter features for efficient parsing, syntax handling, and IntelliSense in config files and more..."
published: true
---
...
![Thumbnail Courtesy https://www.youtube.com/watch?v=1K1bNTOQi5g](/img/tree-sitter.jpg)

*Thumbnail Courtesy https://www.youtube.com/watch?v=1K1bNTOQi5g*
...
![Tree Sitter Capture Blocks color-coded. These are the plucked-out setting names from the AST](/img/code-block-p2.png)

*Tree Sitter Capture Blocks color-coded. These are the plucked-out setting names from the AST*
...
As you can see we get the values inside the `config.get("{setting}")` function call (where `{setting}` is a placeholder) and then go to our LSP to ask for autocompletion values. I won't include every query in this project, but if you are interested, check out the GitHub repository. In practice, this is how my autocompletion looks like:
...
![Text Completion response, for the settings/options present under a particular section](/img/code-block-ac-p2.png)

*Text Completion response, for the settings/options present under a particular section*
